...
template <size_t N>
class POV_Chassis : public ChassisBase<Odom> {
public:
    ...
    void ForwardKinematics() final{
        ...
        for (int i = 0; i < N; ++i){
            SwerveState swerveState_t(modules[i].steerMotor->GetState().position,modules[i].driveMotor->GetState().speed, wheelDiameter, modules[i].zeroPosition);
            states[i] = swerveState_t;
            Xn[i] = alpha * Xn[i] + (1 - alpha) * Matrixf<3, 1>(targetV.data());
        ...
        estimatedV[0] = Xn[0][0][0];
        estimatedV[1] = Xn[0][1][0];
        estimatedV[2] = Xn[0][2][0];
        // memcpy(estimatedV.data(), Xn[0].data(), 3 * sizeof(float));
    }

    void Bind(std::function<void(const float*, const float*)>& _decodeFunc) {
        decodeFunc = _decodeFunc;
    }

    void PubPosAndVel(){
        if(decodeFunc){
            decodeFunc(GetOdom().data(),estimatedV.data());
        }
    }

    void Handle() final {
        ForwardKinematics();
        UpdateOdom(estimatedV);
        // Control
