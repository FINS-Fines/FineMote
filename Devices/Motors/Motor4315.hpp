...
public:
    template<typename T>
    Motor4315(const Motor_Param_t&& params, T &_controller, uint8_t addr) : MotorBase(std::forward<const Motor_Param_t>(params)), id(addr), commuAgent(this, addr) { // Todo: ID和地址分离逻辑
