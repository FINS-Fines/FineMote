#ifndef RS485DEV_BLOW_HPP
#define RS485DEV_BLOW_HPP

#include "RS485Dev.hpp"
// 波特率 1200 2400 4800 9600 19200 38400
// 执行器 blow
template<int busID, int bufSize>
class Blower final : public RS485DevBase<busID, bufSize> {

#define DIVISION (30)
#define CHANNELS (1)
#define LENGTH (8)

public:
    explicit Blower(uint32_t dev_addr, uint8_t reg_addr, uint32_t div = DIVISION)
    : RS485DevBase<busID, bufSize>(dev_addr, div)
    , dev_addr_(dev_addr)
    , reg_addr_(reg_addr) {
        assert(dev_addr > 0);
        assert(dev_addr < 65535);
        assert(reg_addr > 0);
        assert(reg_addr < 255);
    }

    // 设置占空比, 0-100
    void SetComp(const float compare) const {
        assert(compare > 0.0f);
        assert(compare < 100.0f);

        this->curr_comp_ = compare * 10;
    }

protected:
    uint32_t dev_addr_;
    uint8_t reg_addr_;
    float curr_comp_ = 0.0f; // 当前占空比 0-1000

    /// 地址码(1), 功能码(1), 寄存器地址(2), 读取数量(2), CRC16(2)
    void Require() override {
        this->rs485Agent.txbuf[0] = dev_addr_;                                  // 地址码
        this->rs485Agent.txbuf[1] = FunctionCode::WRITE;                        // 功能码 03 读 06 写
        this->rs485Agent.txbuf[2] = 0x00;                                       // 寄存器启始地址[高]
        this->rs485Agent.txbuf[3] = reg_addr_;                                  // 寄存器启始地址[低] =0x08

        // 将占空比转uint16_t
        auto compare = static_cast<uint16_t>(curr_comp_);
        this->rs485Agent.txbuf[4] = (compare & 0x00FF);                         // 占空比[低]
        this->rs485Agent.txbuf[5] = (compare & 0xFF00) >> 8;                    // 占空比[高]

        auto crc16 = CRC16Calc(this->rs485Agent.txbuf, 6);        // CRC16
        this->rs485Agent.txbuf[6] = crc16 & 0x00FF;                             // CRC16[低]
        this->rs485Agent.txbuf[7] = (crc16 & 0xFF00) >> 8;                      // CRC16[高]// 设置发送数据
        this->rs485Agent.SendMsg(8);                                        // 发送数据
    }

    bool Check() override {
        // todo: 检查是否被正确修改，多设备共享同一设备地址可能导致接收消息在不同设备上混淆
        return true;
    }

    /// 响应帧与请求帧完全一致
    void Update() override {
        // todo:
        return;
    }
};

#endif // RS485DEV_BLOW_HPP
