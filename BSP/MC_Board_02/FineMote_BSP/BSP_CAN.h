...
private:
    ...
    void PeriphralInit() {
        ...
        HAL_FDCAN_ActivateNotification(BSP_CANList[ID], FDCAN_IT_TX_COMPLETE,0);
        ...
        fdcanFilter.FilterIndex = (ID == 1) ? 0 : 14;
...
void BSP_CAN<ID>::Receive(FineMote_CAN_HeaderTypeDef *Header, uint8_t *data) {
    ...
    Header->DLC = (FDCAN_Header.DataLength) >> 4;
}
...
void BSP_CAN<ID>::Transmit(FineMote_CAN_HeaderTypeDef *Header, uint8_t *data) {
    ...
    FDCAN_Header.DataLength = (Header->DLC)<<4;//转成FDCAN_data_length_code
