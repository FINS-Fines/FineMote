...
void SPI1_DMA_init(uint32_t tx_buf, uint32_t rx_buf, uint16_t num)
{
    ...
    __HAL_DMA_CLEAR_FLAG(&hdma_spi1_rx, __HAL_DMA_GET_HT_FLAG_INDEX(&hdma_spi1_rx));
    __HAL_DMA_CLEAR_FLAG(&hdma_spi1_rx, __HAL_DMA_GET_TE_FLAG_INDEX(&hdma_spi1_rx));
...
void SPI1_DMA_enable(uint32_t tx_buf, uint32_t rx_buf, uint16_t ndtr)
{
    ...
    /* 数据屏障: 确保所有 DMA 寄存器写入 (地址/计数) 在 DMA 启动前完成 */
    __DSB();
